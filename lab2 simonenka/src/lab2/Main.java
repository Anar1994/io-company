...
class MyTreeModelListener implements TreeModelListener
{
	...
	public void treeNodesChanged(TreeModelEvent e)
	{
		...
		if (Main.isWindows())
		{
			...
			File f = new File(filePath + "\\" + Main.bufName);
			f
					.renameTo(new File(filePath + "\\"
							+ e.getChildren()[0].toString()));
			Main.m_model.reload();
			Main.m_tree.updateUI();
		}
	}
...
class FileTreeCellEditor implements TreeCellEditor
{
	...
	class FileEditor extends JTextField implements CellEditor
	{
		...
		public boolean stopCellEditing()
		{
			// TODO
			String tmp = getText();
			String buf = Main.m_display.getText();
			if (Main.isWindows())
			{
				Main.bufName = buf.substring(buf.lastIndexOf("\\") + 1);
			} else if (Main.isUnix())
			{
